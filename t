...
-- Escape Configs
local NormalEscapeConfig = { Enabled = false, StepSize = 1.4, StepWait = 0.03, ArriveDist = 4, DeathRestart = 5 }
local OOBConfig = { Enabled = false, StepSize = 1.4, StepWait = 0.03, ArriveDist = 4, DeathRestart = 5 }
local activeEscapeRoute = nil
local escapeNoclipConn = nil
local escapePlatform = nil

-- Auto Join Config
local AutoJoinConfig = { Enabled = false }
local autoJoinConn = nil
...
-- ── Section: Patient Functions ────────────────────────────
local PatientIntroSection = PatientTab:Section({ Title = "Patient Functions", Icon = "notebook-text", Box = false, Opened = true })
PatientIntroSection:Paragraph({ Title = "Beta Version", Desc = "Please note this tab is heavily a work in progress; expect possible bugs." })

-- ── Section: Normal Escape ────────────────────────────────
local NormalEscapeSection = PatientTab:Section({ Title = "Normal Escape", Icon = "layout-dashboard", Box = false, Opened = true })
local NormalWaypoints = {
    Vector3.new(223,251,-552), Vector3.new(193,251,-553), Vector3.new(163,251,-550), Vector3.new(126,249,-549),
    Vector3.new(99,249,-547),  Vector3.new(100,249,-520), Vector3.new(104,250,-435), Vector3.new(51,251,-432),
    Vector3.new(49,251,-391),  Vector3.new(155,269,-391), Vector3.new(155,269,-422), Vector3.new(61,287,-422),
    Vector3.new(-11,287,-422), Vector3.new(-78,287,-421), Vector3.new(-77,287,-387), Vector3.new(-76,298,-375),
    Vector3.new(-77,318,-403), Vector3.new(-77,338,-374), Vector3.new(-78,358,-402), Vector3.new(-77,378,-375),
    Vector3.new(-77,398,-403), Vector3.new(-77,418,-374), Vector3.new(-77,438,-402), Vector3.new(-77,458,-375),
    Vector3.new(-77,478,-401), Vector3.new(-76,498,-374), Vector3.new(-78,518,-402), Vector3.new(-77,518,-366),
    Vector3.new(-76,518,-336), Vector3.new(0,518,-337),   Vector3.new(0,518,-195),   Vector3.new(45,518,-196),
    Vector3.new(43,518,-174),  Vector3.new(1,518,-175),   Vector3.new(1,518,-116),   Vector3.new(1,517,-39),
    Vector3.new(1,517,421)
}

-- ── Section: OOB Escape ───────────────────────────────────
local OOBSection = PatientTab:Section({ Title = "OOB Escape", Icon = "route", Box = false, Opened = true })
local OOBWaypoints = {
    Vector3.new(208, 244, -550), Vector3.new(101, 243, -551), Vector3.new(102, 242, -519), Vector3.new(103, 244, -441),
    Vector3.new(77, 243, -443),  Vector3.new(78, 243, -414),  Vector3.new(42, 244, -423),  Vector3.new(46, 245, -391),
    Vector3.new(159, 263, -392), Vector3.new(160, 263, -422), Vector3.new(59, 279, -419),  Vector3.new(-76, 282, -416),
    Vector3.new(-77, 282, -393), Vector3.new(-93, 282, -394), Vector3.new(-96, 368, -384), Vector3.new(-90, 468, -391),
    Vector3.new(-97, 522, -392), Vector3.new(-98, 520, -328), Vector3.new(-91, 518, -316), Vector3.new(-121, 519, -314),
    Vector3.new(-104, 519, -258),Vector3.new(-98, 520, -229), Vector3.new(-93, 517, -191), Vector3.new(-78, 518, -82),
    Vector3.new(-6, 517, -82),   Vector3.new(2, 518, 427)
}

local function stopEscape()
    activeEscapeRoute = nil
    if escapeNoclipConn then escapeNoclipConn:Disconnect(); escapeNoclipConn = nil end
    if escapePlatform then escapePlatform:Destroy(); escapePlatform = nil end
    local char = player.Character
    if char then
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum then
...
local function startEscape(waypoints, config)
    stopEscape()
    config.Enabled = true
    activeEscapeRoute = waypoints

    task.spawn(function()
        while config.Enabled and activeEscapeRoute == waypoints do
            if player.Team and player.Team.Name == "Boxed" then
                task.wait(10)
                continue
            end
            ...
            if escapeNoclipConn then escapeNoclipConn:Disconnect() end
            escapeNoclipConn = RunService.Stepped:Connect(function()
            ...
            if escapePlatform then escapePlatform:Destroy() end
            escapePlatform = Instance.new("Part")
            escapePlatform.Size = Vector3.new(15, 1, 15)
            escapePlatform.Anchored = true
            escapePlatform.Transparency = 1
            escapePlatform.CanCollide = true
            escapePlatform.Name = "OrcaHub_EscapePlatform"
            escapePlatform.Parent = Workspace
            ...
            for i = 1, #waypoints do
                if not config.Enabled or activeEscapeRoute ~= waypoints then break end
                local wp = waypoints[i]

                while config.Enabled and activeEscapeRoute == waypoints do
                    if hum.Health <= 0 or not root.Parent then
                        died = true; break
                    end

                    local pos = root.Position
                    local p2d = Vector2.new(pos.X, pos.Z)
                    local t2d = Vector2.new(wp.X, wp.Z)

                    if (p2d - t2d).Magnitude <= config.ArriveDist then
                        root.CFrame = CFrame.new(wp)
                        if escapePlatform and escapePlatform.Parent then
                            escapePlatform.CFrame = root.CFrame * CFrame.new(0, -3.5, 0)
                        end
                        task.wait(0.04)
                        break
                    end

                    local dir = (wp - pos).Unit
                    local step = math.min(config.StepSize, (wp - pos).Magnitude)
                    root.CFrame = CFrame.new(pos + dir * step) * (root.CFrame - root.CFrame.Position)
            ...
            if died then
                task.wait(config.DeathRestart)
            else
                local ok, result = pcall(function()
                    return game:HttpGet("https://games.roblox.com/v1/games/132352755769957/servers/Public?sortOrder=Asc&limit=100")
                end)
                if ok then
                    local p = HttpService:JSONDecode(result)
                    local servers = {}
                    for _, v in pairs(p.data) do
                        if v.playing < v.maxPlayers and v.id ~= game.JobId then
                            table.insert(servers, v.id)
                        end
                    end
                    if #servers > 0 then
                        TeleportService:TeleportToPlaceInstance(132352755769957, servers[math.random(1, #servers)])
                    else
                        TeleportService:Teleport(132352755769957)
                    end
                else
                    pcall(function() TeleportService:Teleport(132352755769957) end)
                end
                break
            end
        end
...
-- Normal Escape UI
local NormalToggle = NormalEscapeSection:Toggle({
    Title    = "Activate Escape",
    Desc     = "Navigate the normal escape route and server hop on completion.",
    Icon     = "play",
    Value    = false,
    Flag     = "NormalEscapeEnabled",
    Callback = function(state)
        if state then
            if OOBConfig.Enabled then
                OOBConfig.Enabled = false
                OOBToggle:Set(false)
            end
            startEscape(NormalWaypoints, NormalEscapeConfig)
        else
            NormalEscapeConfig.Enabled = false
            stopEscape()
        end
    end,
})

NormalEscapeSection:Input({ Title = "Step Size", Desc = "Studs moved per tick (default: 1.4).", Placeholder = "Default: 1.4", Flag = "NormalStepSize", Callback = function(v) local n = tonumber(v) if n and n > 0 then NormalEscapeConfig.StepSize = n end end })
NormalEscapeSection:Input({ Title = "Step Wait", Desc = "Seconds between ticks (default: 0.03).", Placeholder = "Default: 0.03", Flag = "NormalStepWait", Callback = function(v) local n = tonumber(v) if n and n > 0 then NormalEscapeConfig.StepWait = n end end })
NormalEscapeSection:Input({ Title = "Arrive Distance", Desc = "Distance to consider waypoint reached (default: 4).", Placeholder = "Default: 4", Flag = "NormalArriveDist", Callback = function(v) local n = tonumber(v) if n and n > 0 then NormalEscapeConfig.ArriveDist = n end end })
NormalEscapeSection:Input({ Title = "Death Restart Delay", Desc = "Seconds to wait before restarting after death (default: 5).", Placeholder = "Default: 5", Flag = "NormalDeathRestart", Callback = function(v) local n = tonumber(v) if n and n > 0 then NormalEscapeConfig.DeathRestart = n end end })

-- OOB Escape UI
OOBToggle = OOBSection:Toggle({
    Title    = "Activate Escape",
    Desc     = "Navigate the out-of-bounds escape route and server hop.",
    Icon     = "play",
    Value    = false,
    Flag     = "OOBEscapeEnabled",
    Callback = function(state)
        if state then
            if NormalEscapeConfig.Enabled then
                NormalEscapeConfig.Enabled = false
                NormalToggle:Set(false)
            end
            startEscape(OOBWaypoints, OOBConfig)
        else
            OOBConfig.Enabled = false
            stopEscape()
        end
...
OOBSection:Input({ Title = "Step Size", Desc = "Studs moved per tick (default: 1.4).", Placeholder = "Default: 1.4", Flag = "OOBStepSize", Callback = function(v) local n = tonumber(v) if n and n > 0 then OOBConfig.StepSize = n end end })
OOBSection:Input({ Title = "Step Wait", Desc = "Seconds between ticks (default: 0.03).", Placeholder = "Default: 0.03", Flag = "OOBStepWait", Callback = function(v) local n = tonumber(v) if n and n > 0 then OOBConfig.StepWait = n end end })
OOBSection:Input({ Title = "Arrive Distance", Desc = "Distance to consider waypoint reached (default: 4).", Placeholder = "Default: 4", Flag = "OOBArriveDist", Callback = function(v) local n = tonumber(v) if n and n > 0 then OOBConfig.ArriveDist = n end end })
OOBSection:Input({ Title = "Death Restart Delay", Desc = "Seconds to wait before restarting after death (default: 5).", Placeholder = "Default: 5", Flag = "OOBDeathRestart", Callback = function(v) local n = tonumber(v) if n and n > 0 then OOBConfig.DeathRestart = n end end })

-- ── Section: Patient (Auto Join) ──────────────────────────
local PatientJoinSection = PatientTab:Section({ Title = "Patient", Icon = "refresh-cw", Box = false, Opened = true })

local function joinPatient()
    pcall(function()
        local PATIENT_TEAM = Teams:FindFirstChild("PATIENT")
        local teamChangeRF = ReplicatedStorage:FindFirstChild("TeamChange", true)
        if PATIENT_TEAM and teamChangeRF then
            teamChangeRF:InvokeServer(PATIENT_TEAM, "Patient")
            player:SetAttribute("InMenu", false)
            player:SetAttribute("CurrentTeamIndex", 1)
...
local function startAutoJoin()
    if AutoJoinConfig.Enabled then return end
    AutoJoinConfig.Enabled = true
    joinPatient()
    autoJoinConn = player.CharacterAdded:Connect(function()
        task.wait(1)
        if AutoJoinConfig.Enabled then joinPatient() end
    end)
end

local function stopAutoJoin()
    AutoJoinConfig.Enabled = false
    if autoJoinConn then autoJoinConn:Disconnect(); autoJoinConn = nil end
end

PatientJoinSection:Toggle({
    Title    = "Auto Join Patient",
    Desc     = "Automatically joins the Patient team. Re-fires on respawn.",
    Icon     = "user-plus",
    Value    = false,
    Flag     = "AutoJoinPatientEnabled",
    Callback = function(state)
        if state then startAutoJoin() else stopAutoJoin() end
    end,
})
...
-- ── Section: Work in Progress ─────────────────────────────
local MedicalWIPSection = MedicalTab:Section({ Title = "Work in Progress", Icon = "briefcase-business", Box = false, Opened = true })
MedicalWIPSection:Paragraph({ Title = "Coming Soon", Desc = "We're going to add more awesome features for this tab someday soon." })
...
Players.PlayerRemoving:Connect(function(p)
    if p == player then
        pcall(function()
            ...
            if NormalEscapeConfig.Enabled or OOBConfig.Enabled then stopEscape() end
            if AutoJoinConfig.Enabled then stopAutoJoin() end
        end)
    else
